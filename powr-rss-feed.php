<?php
    ...
    Plugin URI: https://www.powr.io/plugins/rss-feed
    ...
    */
    ...
    class Powr_Rss_Feed extends WP_Widget{
      ...
      public function form( $instance ){
        ...
        ?>
        <p>
          To edit, visit your live webpage and click the gears icon on your RSS Feed.
        </p>
        <p>
    ...
    function powr_rss_feed_menu() {
      add_menu_page( 'POWr RSS Feed', 'POWr RSS Feed', 'manage_options', 'powr-rss-feed-settings', 'powr_rss_feed_options', plugins_url('/powr-icon.png',__FILE__));
    }
    function powr_rss_feed_options() {
      if(powr_local_mode()){//Determine JS url:
        $redirect_url = 'https://localhost:3000/wp-create/rss-feed';
      }else{
        $redirect_url = 'https://www.powr.io/wp-create/rss-feed';
      }
    ...
    register_activation_hook(__FILE__, 'powr_rss_feed_plugin_activate');
      add_action('admin_init', 'powr_rss_feed_plugin_redirect');

      function powr_rss_feed_plugin_activate() {
      add_option('powr_rss_feed_plugin_do_activation_redirect', true);
      }
      ...
      function powr_rss_feed_plugin_redirect() {
      if (get_option('powr_rss_feed_plugin_do_activation_redirect', false)) {
          delete_option('powr_rss_feed_plugin_do_activation_redirect');
          if(!isset($_GET['activate-multi']))
          {
            wp_redirect( get_admin_url().'?platform=wordpress&page=powr-rss-feed-settings&' );
          }
       }
      }
